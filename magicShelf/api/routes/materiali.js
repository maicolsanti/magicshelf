...
router.get('/:codice_materiale', async (req, res) => {
    ...
    try {
      const [rows] = await pool.execute('SELECT * FROM ANAGRAFICA_MATERIALE WHERE CODICE_MATERIALE = ?', [codice_materiale]);
...
router.delete('/:codice_materiale', async (req, res) => {
  ...
  const query = 'DELETE FROM ANAGRAFICA_MATERIALE WHERE CODICE_MATERIALE = ?';
